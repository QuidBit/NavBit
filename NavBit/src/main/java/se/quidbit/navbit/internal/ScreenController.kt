...
internal sealed class ScreenPreInitializedState {
    class InProgress(val onDone : ((NavBitScreen<*>) -> Unit)?) : ScreenPreInitializedState()
    class Done(val navBitScreen : NavBitScreen<*>) : ScreenPreInitializedState()
}

internal object ScreenController {
    private val backstack   = ArrayList<Pair<NavBitScreen<*>, ScreenType>>()
    private val toBeRemoved = ArrayList<Pair<NavBitScreen<*>, ScreenType>>()
        ...
        when (direction) {
                ...
                preInitializedScreens[id]?.let { state ->
                    when (state) {
                        ...
                        is ScreenPreInitializedState.Done -> {
                            ...
                            backstack.add(Pair(state.navBitScreen, type))
                            completeGoToScreen(state.navBitScreen, transition, true, mainContainer, screenData, onCompletion)
            ...
            TransitionDirection.Backward -> {
                ...
                if (!backStackContains<D>(screenData.tag(), type)) {
                    ...
                    var foundNavBitScreen : NavBitScreen<*>? = null
            ...
            if (newlyAdded) {
                ...
                NavBitActivity.mainHandler.post {
                    ...
                    ViewCompat.getRootWindowInsets(mainContainer)?.let { startInsets ->
                        ViewCompat.dispatchApplyWindowInsets(navBitScreen, startInsets)
                    }
    ...
    fun getCurrentScreen() : NavBitScreen<*> {
    ...
    fun cleanupScreens(lifecycleScope : LifecycleCoroutineScope, mainContainer : FrameLayout) {
        ...
        lifecycleScope.launch(Dispatchers.IO) {
            ...
            delay(totalDelay)
            Log.i("NavBit-ScreenController", "Screens Cleanup done - Removed: $removedCount - (toBeRemoved left: ${toBeRemoved.size}, backstatck size : ${backstack.size})")
        }
    }

    fun getBackgroundScreens() : ArrayList<NavBitScreen<*>> {
        val result = ArrayList<NavBitScreen<*>>()
        var first = true
    ...
    fun <S : NavBitNavigationState, D : NavBitScreenData>restoreScreens(
        ...
        Log.i("NavBit-ScreenController", "Restoring ${toBeRestored.size} screens")

        val restored  = HashMap<Int, Pair<NavBitScreen<*>, ScreenType>>()

        var currentScreen = 0

        // Start the generation of each screen
        for ((oldData, screenType) in toBeRestored.reversed()) {
            Log.i("NavBit-ScreenController", "    --- $oldData - $screenType")
                ...
                NavBitActivity.mainHandler.post {
                    newScreen.restoreScreen(oldData) {
                        ...
                        if (restored.size == toBeRestored.size) {
                            ...
                            toBeRestored.clear()
                            Log.i("NavBit-ScreenController", "Restoring complete")
